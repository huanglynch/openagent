    ...
    def _solve_complex(
        ...
        history.append({
            ...
            "content": (
                ...
                "**重要输出要求（必须严格遵守）：**\n"
                "1. **始终使用清晰、优美、自然的 Markdown 格式**输出（使用标题、列表、表格、加粗、代码块、分隔线等），让用户直接复制阅读最舒服。\n"
                "2. **不要把整个答案包装成 JSON**，不要输出卡片格式作为主体。\n"
                "3. 只有当内容**非常适合结构化展示**时，才在**答案最末尾**附加一个简洁的 Markdown 代码块卡片。\n"
                "4. 优先保证整体美观和可读性。\n\n"

                # ==================== 新增：文件下载强制要求（最小改动核心） ====================
                "**📥 文件交付要求（用户明确需求时强制执行）**：\n"
                "如果本次任务中通过工具（write_file / code_executor 等）**成功修改或生成了文件**（尤其是用户上传的 Excel、报告、PDF 等），"
                "请在答案**最末尾**单独成段添加下载链接：\n\n"
                "**📥 下载修改后的文件**： [点击下载](/uploads/实际保存的文件名)\n\n"
                "文件名必须来自工具返回的 Observation 中的路径（例如 `uploads/modified_report_xxx.xlsx` 中的文件名部分）。"
                "链接使用相对路径 `/uploads/...` 即可（WebUI 和飞书均可直接点击）。"
                "如果有多个文件请全部列出。绝不遗漏。\n\n"
                # =============================================================================
